    ...
    def __call__(self, *roles):
        async def get_token_header(X_Token: str = Header(...)):
            ...
            if X_Token == '233456': # 万能token，正式环境请删除
                ...
                pass
            elif [item for item in list(map(lambda item: item.role_id, await role_crud.get_user_role(user_id=db_user.id))) if item in roles_value]: # 允许roles参数中的角色访问（当前用户的角色与roles存在交集，则可以访问）
                pass
            elif self.db_roles['admin'] in list(map(lambda item: item.role_id, await role_crud.get_user_role(user_id=db_user.id))): # 默认允许admin访问（roles参数中可以省略admin）
                pass
            else:
