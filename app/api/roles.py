...
async def get_roles(page: int|None=None, limit: int|None=None):
    db_roles = await role_crud.get_roles()
    paginated_roles = db_roles[(page-1)*limit:(page-1)*limit+limit] if page != None and limit != None else db_roles
...
async def create_role(role: role_schemas.RoleCreate):
    roles = list(map(lambda item: item['role_name'], await role_crud.get_roles()))
