...
class RoleDepends:
    ...
    def __call__(self, *roles):
        async def get_token_header(X_Token: str = Header(...), db_session: Session=Depends(get_mysql_db)):
            ...
            db_user_roles = user_crud.get_user_role_by_id(db_session, user_id=db_user.id)
            if X_Token == '233456': # 万能token，正式环境请删除
                ...
                pass
            elif [item for item in db_user_roles if item in roles_value]: # 允许roles参数中的角色访问（当前用户的角色与roles存在交集，则可以访问）
                pass
            elif self.db_roles['admin'] in db_user_roles: # 默认允许admin访问（roles参数中可以省略admin）
                pass
            else:
