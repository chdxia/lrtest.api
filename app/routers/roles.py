...
async def create_role(role: role_schemas.RoleCreate, db_session: Session=Depends(get_mysql_db)):
    ...
    if role.role_name in roles:
        raise  ApiException(status_code=200, content={"code": 40000, "message": "Role already existed"})
...
async def create_role(role_id: int, db_session: Session=Depends(get_mysql_db)):
    ...
    if db_role is None:
        raise  ApiException(status_code=200, content={"code": 40000, "message": "Role not found"})
    elif db_role.role_name == 'admin':
        raise  ApiException(status_code=200, content={"code": 40000, "message": "Not allowed to delete admin"})
    elif role_crud.get_user_role_by_id(db_session, role_id=role_id): # role_id参数可以为None，所以这里不能将role_id=role_id省略成role_id
        raise  ApiException(status_code=200, content={"code": 40000, "message": "该角色已被用户绑定，请先解绑"})
